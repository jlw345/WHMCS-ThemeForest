...
<div id="pricing" class="row row-product-selection">
    ...
    <div class="col-xs-12">
        ...
        <div id="order-hostio_cart" class="page-container">
                ...
                <ul id="sub-pricing">
                    {foreach $products as $product}
                        ...
                        <li id="product{$product@iteration}">
                            <div class="pricing-box blue-after {if $product.isFeatured} best-seller{/if}">
                                <div class="pricing-icon"></div>
                                <div class="pricing-title" id="product{$product@iteration}-name">{$product.name}</div>
                                {if $product.tagLine}
                                    <p id="product{$product@iteration}-tag-line">{$product.tagLine}</p>
                                {/if}
                                <div class="pricing-amount" id="product{$product@iteration}-price">
                                    <div class="price">
                                    {if $product.bid}
                                        {$LANG.bundledeal}
                                ...
                                <div class="pricing-button">
                                    {if $product.qty eq "0"}
                                        <a class="blue-button unavailable" href="#" id="product{$product@iteration}-unavailable">{$LANG.outofstock}</a>
                                    {else}
                                        <a class="blue-button" href="{$smarty.server.PHP_SELF}?a=add&amp;{if $product.bid}bid={$product.bid}{else}pid={$product.pid}{/if}" id="product{$product@iteration}-order-button">
                                            {$LANG.ordernowbutton}
                                        </a>
                                    {/if}
                                </div>
